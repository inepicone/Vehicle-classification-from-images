...
# to validate the experiment configuration:
...
# to load from a YAML to a dictionary the experiment configuration 
...
# function to map class names and probabilty (output of the model)
...
def predict_from_folder(folder, model, input_size, class_names):
    ...
    # we return a tuple of two lists:
    labels = []
    predictions = []

    # 1) walk through all the directory :

    for file_path, img_name in walkdir(folder):
        # I create the path to the image so I can load it
        ...
        # getting the predictions: prediction without s
        prediction = model.predict(img_array)
    
        class_pred = class_names[np.argmax(prediction)]
        predictions.append(class_pred)

        # I need to get the correct label. I can get it from the folder (that is the path) 
        _, correct_label = os.path.split(file_path)
