...
def main(data_folder, labels, output_data_folder):
    ...
    with open(labels,'r') as csv_file:
        ...
        for row in csv_r:
            ...
            if r_count > 0:
                sub_folder           = row[2]
                subset_folder        = os.path.join(output_data_folder,sub_folder)

                if (not os.path.isdir(subset_folder)):
                    os.mkdir(subset_folder)
                
                class_folder         = row[1] 
                class_folder_path    = os.path.join(subset_folder,class_folder)
                
                if (not os.path.isdir(class_folder_path)):
                    os.mkdir(class_folder_path)
                
                img_name           = row[0]
                img_name_path_old  = os.path.join(data_folder       , img_name)
                img_name_path_new  = os.path.join(class_folder_path , img_name)
                
                if (not os.path.exists(img_name_path_new)):
                                        
                    os.link( img_name_path_old , img_name_path_new) 
            r_count+=1
