...
def parse_regulizers(config):
    """
    Add Keras regulizers based on experiment settings.
    ...
    """
    regulizers = []
    if "regulizers" in config["model"]:
        for regulizer_name, regulizer_params in config["model"]["regulizers"].items():
            regulizers.append( REGULIZERS[regulizer_name](**regulizer_params))

        del config["model"]["regulizers"]
